...
class LinearOperations(object):
    def __init__(self, fname):
        '''
        an image object. Loads in some data from jpg and converts it to useful
        '''
    ...
    def fft2(self):
        ...
        '''
        tmp = self.image
        self.data = np.fft.fft2(np.fft.fftshift(tmp))
    ...
    def median(self, kernel):
        ...
        '''
        tmp = self.image
        self.data = nd.median(tmp, size=kernel)
    ...
    def gaussian(self, sigma):
        ...
        '''
        tmp = self.image
        self.data = nd.gaussian_filter(tmp, sigma)
    ...
    def sobel(self):
        ...
        '''
        tmp=self.image
        self.data = nd.sobel(tmp)
        
    def affine_transform(self, matrix=[5,5]):
        tmp = self.image
        self.data = nd.affine_transform(tmp, np.array(matrix))
